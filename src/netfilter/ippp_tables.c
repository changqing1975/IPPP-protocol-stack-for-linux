...
// static inline bool
// ip_packet_match(const struct iphdr *ip, const char *indev, const char *outdev,
//         const struct ipt_ip *ipinfo, int isfrag) {
//     unsigned long ret;
...
// // static unsigned int ipt_error(struct sk_buff *skb, const struct xt_action_param *par) {
...
//     return e->target_offset == sizeof(struct ipt_entry) && memcmp(&e->ip, &uncond, sizeof(uncond)) == 0;
// }
...
// /* Mildly perf critical (only if packet tracing is on) */
// static inline int get_chainname_rulenum(const struct ipt_entry *s, const struct ipt_entry *e,
...
//             const struct sk_buff *skb, unsigned int hook,
...
//         if (get_chainname_rulenum(iter, e, hookname, &chainname, &comment, &rulenum) != 0)
//             break;
...
//     const char *indev = state->in ? state->in->name : nulldevname;
//     const char *outdev = state->out ? state->out->name : nulldevname;
...
//     const struct xt_table_info *private = READ_ONCE(table->private); /* Address dependency. */
//     unsigned int cpu        = smp_processor_id();
//     const void *table_base = private->entries;
//     struct ipt_entry **jumpstack  = (struct ipt_entry **)private->jumpstack[cpu];
...
//     struct ipt_entry *e = get_entry(table_base, private->hook_entry[hook]);
...
//         struct xt_counters *counter = xt_get_this_cpu_counter(&e->counters);
//         ADD_COUNTER(*counter, skb->len, 1);

//         const struct xt_entry_target *t = ipt_get_target_c(e);
...
//             int v = ((struct xt_standard_target *)t)->verdict;
//             if (v < 0) {
//                 /* Pop from stack? */
//                 if (v != XT_RETURN) {
...
//                 else
//                     e = ipt_next_entry(jumpstack[--stackidx]);
//                 continue;
//             }
...
// //             unsigned int valid_hooks, void *entry0, unsigned int *offsets)
// //  {
// //      unsigned int hook;
...
// static void __ipppt_unregister_table(struct net *net, struct xt_table *table) {
...
// int ipppt_register_table(struct net *net, const struct xt_table *table,
//             const struct ipt_replace *repl, const struct nf_hook_ops *template_ops) {
 
//     struct xt_table_info *newinfo = xt_alloc_table_info(repl->size);
//     if (!newinfo)
//         return -ENOMEM;

//     void *loc_cpu_entry = newinfo->entries;
//     memcpy(loc_cpu_entry, repl->entries, repl->size);
 
//     int ret = translate_table(net, newinfo, loc_cpu_entry, repl);
//     if (ret != 0) {
//         xt_free_table_info(newinfo);
//         return ret;
//     }
...
// //     struct xt_table *new_table = xt_register_table(net, table, &bootstrap, newinfo);
// //     if (IS_ERR(new_table)) {
// //         struct ipt_entry *iter;
...
// //     unsigned int num_ops = hweight32(table->valid_hooks);
// //     if (num_ops == 0) {
// //         ret = -EINVAL;
// //         goto out_free;
// //     }

// //     struct nf_hook_ops *ops = kmemdup_array(template_ops, num_ops, sizeof(*ops), GFP_KERNEL);
// //     if (!ops) {
// //         ret = -ENOMEM;
// //         goto out_free;
// //     }
...
// // out_free:
// //     __ipppt_unregister_table(net, new_table);
//     return ret;
// }
...
// //      if (table)
// //          __ipppt_unregister_table(net, table);
// //  }
...
// static int __net_init ippp_tables_net_init(struct net *net)
// {
//     return 0/* xt_proto_init(net, NFPROTO_IPV4) */;
// }
 
// static void __net_exit ippp_tables_net_exit(struct net *net)
// {
// //  xt_proto_fini(net, NFPROTO_IPV4);
// }
 
// static struct pernet_operations ippp_tables_net_ops = {
//     .init = ippp_tables_net_init,
//     .exit = ippp_tables_net_exit,
// };
 
// static int __init ip_tables_init(void) {
//     int ret;

//     ret = register_pernet_subsys(&ippp_tables_net_ops);
//     if (ret < 0)
//         goto err1;
...
// //  err2:
// //      unregister_pernet_subsys(&ippp_tables_net_ops);
// err1:
//     return ret;
// }
 
// static void ip_tables_fini(void) {
...
/* 
如何在Linux内核中执行某些用户态程序或系统命令？在用户态中，可以通过execve()实现；在内核态，
则可以通过call_usermodehelpere()实现该功能。如果您查阅了call_usermodehelper()内核函数的源码实现，
就可以发现该函数最终会执行do_execve()。而execve系统调用在经历内核的系统调用流程后，也会最终调用do_execve()。

#include <linux/kernel.h>
#include <linux/module.h>
#include <linux/init.h>
#include <linux/kmod.h> // 包含调用用户空间帮助器的头文件
